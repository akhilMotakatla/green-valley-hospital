...
import { listStaffContactMessages, setStaffContactMessageStatus } from '../../api/staff';
import type { ContactMessage } from '../../types';
import { extractErrorMessage } from '../../api/client';
import { formatDateTime } from '../../utils/format';
import { StatusBadge } from '../../components/StatusBadge';

export function StaffContactMessagesPage() {
  const [items, setItems] = useState<ContactMessage[]>([]);
  const [statusFilter, setStatusFilter] = useState('');
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(() => {
    listStaffContactMessages({ status: statusFilter || undefined, page: 1, page_size: 50 })
  ...
  async function handleStatus(id: number, status: 'Reviewed' | 'Resolved') {
    try {
      await setStaffContactMessageStatus(id, status);
      load();
    } catch (err) {
